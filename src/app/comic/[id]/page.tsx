...
export default function ComicDetailPage() {
  ...
  const [item, setItem] = useState<ComicItem | null>(null);
  ...
  useEffect(() => {
    const fetchComicItem = async () => {
      try {
        const response = await fetch(`/api/comic/${params.id}`);
        if (!response.ok) {
          throw new Error('Comic item not found');
        }
        const data = await response.json();
        setItem(data);
      } catch (err) {
        console.error('Error fetching comic item:', err);
        setError('コミックアイテムの取得に失敗しました');
      } finally {
        setLoading(false);
      }
    };
          ...
          <div className="max-w-7xl mx-auto">
            ...
            <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-4">
              {(item.imageUrls && item.imageUrls.length > 0 ? item.imageUrls : [item.thumbnail]).map((imageUrl, index) => (
                <div 
                  key={index} 
                  className="aspect-[3/4] rounded-lg overflow-hidden cursor-pointer hover:opacity-80 transition-opacity duration-300 border border-white/20 hover:border-white/40 relative"
          ...
          <div className="fixed inset-0 bg-black/90 flex items-center justify-center z-50">
            <div className="relative w-full h-full flex items-center justify-center p-4">
              ...
              <img
                ...
                alt={`${item.title} - Page ${selectedImageIndex + 1}`}
              ...
              {(item.imageUrls && item.imageUrls.length > 1) && (
                <div className="absolute bottom-4 left-1/2 transform -translate-x-1/2 px-4 py-2 bg-black/70 rounded-full text-white text-sm">
                  Page {selectedImageIndex + 1} / {item.imageUrls.length}
                </div>
              )}
            </div>
          </div>
